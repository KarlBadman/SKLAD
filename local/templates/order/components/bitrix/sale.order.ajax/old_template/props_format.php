...
if (!function_exists("PrintPropsForm"))
{
	...
	function PrintPropsForm($arSource = array(), $locationTemplate = ".default",$arParams = array(),$user_type = 1,$arError = array(),$span = 'N',$auto = false)
	{
		...
		if (!empty($arSource))
		{
			?>
					...
					{
					    if(array_search($arProperties['ID'], $arParams) !== false):?>

						<div class="field__widget type-inline" data-property-id-row="<?=intval(intval($arProperties["ID"]))?>">
						...
						{?>
						    <?if($arProperties["CODE"] == 'F_CITY' || $arProperties["CODE"] == 'U_CITY'):?>
                                <div class="field__widget type-inline">
                                    <label for="input-about-city" class="label"><?=$arProperties['NAME']?>:</label>
                                    <div class="field">
                                        <div class="input">
                                            <?
                                            global $APPLICATION;
                                            $APPLICATION->IncludeComponent(
                                                ...
                                                array(
                                                    ...
                                                    'PROP_NAME'=>$arProperties["FIELD_NAME"]
                                                ),
                                                false
                                            );
                                            ?>
                            ...
                            <?else:?>
                                <label for="<?=$arProperties["FIELD_NAME"]?>" class="label">
                                    <span class="switcher-legal"><?=$arProperties["NAME"]?>:</span>
                                </label>
                                <div class="field">
                                    <?if(!empty($arError)){
                                        $error = false;
                                        foreach ($arError as $errorVal){
                                            if(strripos($errorVal, $arProperties["NAME"]) !== false){
                                                $error = true;
                                            }
                                        }
                                    }else{
                                    ...
                                    <div class="input <?if($arProperties['REQUIRED'] == 'Y'&& $error) echo 'error';?>">
                                        <input
                                                ...
                                                <?elseif ($arProperties['IS_EMAIL'] == 'Y'):?>
                                                    data-name="is_email"
                                                <?elseif ($arProperties['IS_ADDRESS'] == 'Y'):?>
                                                    data-name="is_address"
                                                <?endif;?>
                                        />
                                    </div>
                                    <?if($arProperties['IS_EMAIL'] == 'Y'):?>
                                        <label class="label__widget">
                                            <span class="row">
                                                <span class="control">
                                                    <input type="checkbox" name="subscrible" value="Y" autocomplete="off" checked="<?if($span == 'Y')echo 'checked'?>">
                                                    <u class="square">
                                ...
                                <?if($arProperties['ID'] == 10):?>
                                    <label class="label__widget close_yurik" for="PERSON_TYPE_1">
                                        <svg><use xlink:href="/local/templates/dsklad/images/sprite.svg#cross"></use></svg><span class="row">
                                            <span class="control">
                                                <input class="switcher" type="radio" id="PERSON_TYPE_1" name="PERSON_TYPE" value="1" onclick="submitForm()" style="display: none">
                                            </span>
                                    </label>
                                <?endif;?>
                            <?endif;?>
