...
if (!window.BX_YMapAddPlacemark) {
  ...
  window.BX_YMapAddPlacemark = function(map, arPlacemark) {
    ...
    globalPlacemarks.push(obPlacemark);

    globalMap = map;

    var index = arPlacemark.INDEX;

    obPlacemark.events.add('click', function() {
      $('.scrollable .item.active').removeClass('active');
      var scrollPosition = $('.scrollable .item').eq(index).addClass('active').position().top + $('.scrollable').scrollTop();
      $('.scrollable').animate({scrollTop: scrollPosition+"px"});
    });
