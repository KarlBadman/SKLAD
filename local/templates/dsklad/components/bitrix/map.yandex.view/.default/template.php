...
if ($arParams['BX_EDITOR_RENDER_MODE'] == 'Y') {
	...
	$arTransParams = array(
		...
		'MAP_NEW_HEIGHT' => $arParams['MAP_NEW_HEIGHT'],
    ...
    <script>
        ...
        function BX_SetPlacemarks_<?= $arParams['MAP_ID'] ?>(map)
        {
            if (typeof window['BX_YMapAddPlacemark'] != 'function') {
                ...
                */
                ...
                var ymWaitIntervalId = setInterval(function() {
                    if (typeof window['BX_YMapAddPlacemark'] == 'function') {
                        BX_SetPlacemarks_<?= $arParams['MAP_ID'] ?>(map);
            ...
            if (is_array($arResult['POSITION']['PLACEMARKS']) && ($cnt = count($arResult['POSITION']['PLACEMARKS']))) {
                for ($i = 0; $i < $cnt; $i++) {
                    ?>
                    arObjects.PLACEMARKS[arObjects.PLACEMARKS.length] = BX_YMapAddPlacemark(map, <?= \CUtil::PhpToJsObject(array_merge($arResult['POSITION']['PLACEMARKS'][$i], array('INDEX' => $i))) ?>);
                    <?
                }
            }
            ...
            if (is_array($arResult['POSITION']['POLYLINES']) && ($cnt = count($arResult['POSITION']['POLYLINES']))) {
                for ($i = 0; $i < $cnt; $i++) {
                    ?>
                    arObjects.POLYLINES[arObjects.POLYLINES.length] = BX_YMapAddPolyline(map, <?= \CUtil::PhpToJsObject($arResult['POSITION']['POLYLINES'][$i]) ?>);
                    <?
                }
            }
