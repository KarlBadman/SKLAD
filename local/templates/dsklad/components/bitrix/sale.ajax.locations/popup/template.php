...
if ($arParams["AJAX_CALL"] != "Y"
    ...
    foreach ($arParams["LOC_DEFAULT"] as $val):
        ...
        <div><input onChange="<?= $arParams["ONCITYCHANGE"] ?>;" <?= $checked ?> type="radio"
                    ...
                    id="loc_<?= $val["ID"] ?>"/><label
                for="loc_<?= $val["ID"] ?>"><?= $val["LOC_DEFAULT_NAME"] ?></label></div>
    <? endforeach; ?>
    <div><input <? if ($isChecked != "Y") echo 'checked'; ?> type="radio" onclick="clearLocInput();"
                                                             name="NEW_LOCATION_<?= $arParams["ORDER_PROPS_ID"] ?>" value="0"
                                                             id="loc_0"/><label
            for="loc_0"><?= GetMessage("LOC_DEFAULT_NAME_NULL") ?>:</label></div>
<? endif; ?>
...
<input
    ...
    autocomplete="off"
    placeholder="Напр. Санкт-Петеребург"
