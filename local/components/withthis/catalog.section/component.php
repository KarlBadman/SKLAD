...
$arrWithThis = array();
$dbResWithThis = CIBlockElement::GetProperty($arParams["IBLOCK_ID"], $arParams["ELEMENT_ID"], array(), array("CODE"=>"WITH_THIS"));
while($arTmpWithThis = $dbResWithThis->GetNext()) 
{
	if (isset($arTmpWithThis['VALUE'])) {$arrWithThis[] = $arTmpWithThis['VALUE'];}
}

$offer_IDs=array();
if (count($arrWithThis) < 1)  {
	foreach ($arParams["ELEMENT_OFFERS"] as $arOffer) {$offer_IDs[] = $arOffer["ID"];} 

	        $order_IDs=array();
		$dbOrderIDs = CSaleBasket::GetList(array(), array(">ORDER_ID" => "0", "PRODUCT_ID" => $offer_IDs), false, false, array("ORDER_ID"));
		while ($arOrderID = $dbOrderIDs->GetNext()) {
			$order_IDs[] = $arOrderID["ORDER_ID"];
		}

		$arF = array("ORDER_ID" => $order_IDs, "!PRODUCT_ID" => $offer_IDs);
		$arGroupBy = array("PRODUCT_ID", "SUM" => "QUANTITY");
		$arSelectFields = array("PRODUCT_ID"); 
		$dbBasketItems = CSaleBasket::GetList(array("QUANTITY" => "DESC"), $arF, $arGroupBy, false, $arSelectFields);
		// сортировка по сумме не работает :(
		$j = 0;
		$max_elem = (int)$arParams["MAX_COUNT"];
		while ($arBasketItem = $dbBasketItems->GetNext()) {
			if ($j < $max_elem) {
			$itemWithThis = CCatalogSku::GetProductInfo($arBasketItem["PRODUCT_ID"]);
			$arrWithThis[] = $itemWithThis["ID"];
			$j++; }
		}     
}
if (count($arrWithThis) < 1)  {    // empty
	$arrWithThis[] = -1;
}
...
$arrFilter = array_merge($arrFilter, array("ID" => $arrWithThis));
