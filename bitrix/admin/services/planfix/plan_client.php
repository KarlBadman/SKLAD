...
class Planfix_API {
    ...
    protected function importParams($requestXml, $params,$array = false) {
        foreach($params as $key => $val) {
            ...
            if (is_array($val)) {
                ...
                foreach($val as $key2 => $val2) {
                    // 2 level
                    if (is_array($val2)) {
                        $requestXml->$key->$key2 = new SimpleXMLElement("<$key2/>");
                        foreach($val2 as $key3 => $val3) {
                            // 3 level
                            if (is_array($val3)) {
                                $requestXml->$key->$key2->$key3 = new SimpleXMLElement("<$key3/>");
                                foreach($val3 as $key4 => $val4) {
                                    // 4 level
                                    $requestXml->$key->$key2->$key3->addChild($key4, $val4);
                                }
                            }else{
                                $requestXml->$key->$key2->addChild($key2, $val2);
                            }
                        }
                    } else {
...
//    }
    ...
    protected function exportData($responseXml) {
        ...
        if (get_class($responseXml) == 'SimpleXMLElement') {
            ...
            $x = $responseXml;
            $responseXml = get_object_vars($responseXml);
        }
        
        if (is_array($responseXml)) {
            
            if (count($responseXml) == 0) return (string) $x;
            
            foreach($responseXml as $key => $value) {
                $r[$key] = $this->exportData($value);
            }
